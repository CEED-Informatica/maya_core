...
class SchoolYear(models.Model):
  ...
  def _calculate_task(self):
    ...
    _logger.info('Creando cron por ciclo de manera independiente => todos los módulos menos tutoría')
    for course in courses:
      ...
      for subject in course.subjects_ids:
        ...
        if len(classroom_id) == 0:  # descarto los módulos que no tienen asociada aula virtual
    ...
    for record in self:
      ...
      courses_id = [x.id for x in self.env['maya_core.course'].search([])]
      courses = self.env['maya_core.course'].browse(courses_id)  
      cron_ids = []
      ...
      for course in courses:
        ...
        for subject in distinct_subject_tut:   
          ...
          task = (0, 0, {
            'model_id': record.env.ref('maya_core.model_maya_core_classroom'),
            'name': 'Notifica estado convalidaciones erroneas {} en Aules {}'.format(course.abbr, 
              '/{}'.format(subject.year) if len(list(distinct_subject_tut)) > 1 else ''),
            'active': True,
            'interval_number': 1,
            'interval_type': 'hours',
  ...
  def school_year_to_current_action(self):
    ...
    school_year = self.env['maya_core.school_year'].search([('state', '=', '1')])
