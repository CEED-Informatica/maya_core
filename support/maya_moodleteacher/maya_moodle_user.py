...
class MayaMoodleUser(MoodleUser):
  """
  ...
  def from_raw_json(cls, raw_json):
    """
    Crea un :class:`MayaMoodleUser` objeto desde un JSON
    """
...
class MayaMoodleUsers(list):
  """
  Una lista de instancias de la :class:`MayaMoodleUser`.
  """
  ...
  def from_course(cls, conn, course_id, only_students = False):
    """
    Genera una lista de participantes de un curso (de un aula virtual, classroom para maya)
    """
    obj = cls()
    ...
    for st in response:
      ...
      student = MayaMoodleUser.from_raw_json(st_json)
      obj.append(student)
